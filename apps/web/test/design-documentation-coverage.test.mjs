...
test('設計書カバレッジの初期評価は不足証拠を黄色として具体的に示す', async () => {
  const html = await readFile(coveragePath, 'utf8');
  const scores = [...html.matchAll(/scores:\[([^\]]+)\]/g)].map((match) => match[1].replace(/\s/g, ''));
  assert.equal(scores.length, 10);
  assert.ok(scores.some((score) => score.includes('3') || score.includes('4')));
  assert.ok(scores.every((score) => score.split(',').every((value) => ['3', '4', '5'].includes(value))));
  assert.match(html, /一次棚卸しでは正本リンクの存在を確認したが/);
  assert.match(html, /外部境界と回帰証跡/);
  assert.match(html, /実データの受入未達はP0完成度星取表で別管理/);
  assert.match(html, /実機UXの未達はP0完成度星取表で別管理/);
});
