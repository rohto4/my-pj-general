...
test('設計書カバレッジの初期評価は機能再現の根拠がそろった5点を示す', async () => {
  ...
  assert.match(html, /現行P0はRaw event storeを使わず/);
...
test('今回補完した契約は現行P0の事実と未実装P1の開始境界を分ける', async () => {
  ...
  assert.match(source, /現行P0では独立した Raw入口イベント \/ 正規化イベントの永続storeは実装していない/);
...
test('設計書化レビュー結果表は10機能のファイル量・分割・着眼点を示す', async () => {
  const summary = await readFile(reviewSummaryPath, 'utf8');
  assert.equal((summary.match(/<tr>/g) ?? []).length - 1, 10);
