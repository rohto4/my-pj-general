...
test('設計書カバレッジから辿る正本とskillのローカルリンクは全て存在する', async () => {
  ...
  assert.ok(links.includes('../../apps/web/prompts/threadline-candidate-proposal-v2.txt'));
...
test('共通v2の実行位置とVault sequenceは閲覧themeに依存せず再現できる', async () => {
  ...
  assert.match(contract, /system = 共通v2/);
