...
test("AI相談はローカルLLM停止時に入力口とサイド窓口を閉じる", async () => {
  const app = await readFile(join(webRoot, "app.js"), "utf8");

  assert.match(app, /function chatIsAvailable\(\)/);
  assert.match(app, /chatPageInput[\s\S]{0,180}disabled\s*=\s*!available/);
  assert.match(app, /chatDrawerInput[\s\S]{0,180}disabled\s*=\s*!available/);
  assert.match(app, /if \(!chatIsAvailable\(\)\) \{[\s\S]{0,120}closeChatDrawer\(\)/);
});
...
test("P0 管理データはSQLiteに保存され、初期データにモック候補を含めない", async (context) => {
  ...
  const updatedPrompt = await request("/api/admin/prompt-templates/candidate-triage", {
    method: "PATCH",
    body: JSON.stringify({
      enabled: true,
      body: "未完了の次アクションだけを候補化し、見出しや完了記録をタスク化しない。",
    }),
  });
  assert.equal(updatedPrompt.body, "未完了の次アクションだけを候補化し、見出しや完了記録をタスク化しない。");
...
test("ローカルLLM相談は会話を保存し、候補を確認待ちへ追加できる", async (context) => {
  ...
  const fakeLlm = createServer(async (request, response) => {
    ...
    response.writeHead(200, { "content-type": "application/json" });
    if (requestBody.messages?.[0]?.content?.includes("source-neutral intake analyst")) {
      const sourceBody = requestBody.messages.at(-1).content.split("SOURCE_BODY:\n")[1];
      response.end(JSON.stringify({
        choices: [{
          message: {
            role: "assistant",
            content: JSON.stringify({
              document_summary: "相談を実装タスクとして整理したいという依頼。",
              candidate_proposals: [{
                proposal_type: "action",
                title: "相談内容を実装タスクとして整理する",
                summary: "相談内容を実装候補として整理する。",
                todo: "相談内容を実装タスクとして整理する",
                kind: "todo",
                schedule: "候補なし",
                confidence: "high",
                missing: [],
                tags: [],
                evidence_quotes: [sourceBody],
              }],
            }),
          },
        }],
    ...
    response.end(JSON.stringify({
      choices: [{
        message: {
          role: "assistant",
          content: "相談内容を整理しました。候補として確認できます。",
        },
      }],
    }));
  });
  ...
  assert.match(received[0].messages.find((message) => message.role === "system").content, /候補抽出は回答後に別の共通validator/);
  assert.equal(received[0].messages.at(-1).content, "この相談を実装タスクとして整理したい");
  assert.match(received[1].messages[0].content, /aspiration/);
  assert.match(received[1].messages.at(-1).content, /SOURCE_KIND: chat/);
  assert.match(received[1].messages.at(-1).content, /SOURCE_BODY:\nこの相談を実装タスクとして整理したい$/);
...
test("ローカルLLM停止中のAI相談は送信前に閉じ、会話を保存しない", async (context) => {
  const dir = await mkdtemp(join(tmpdir(), "pj-general-chat-closed-"));
  const appPort = 4198;
  const child = spawn(process.execPath, [join(webRoot, "server.mjs")], {
    cwd: webRoot,
    env: {
  ...
  const bootstrap = await (await fetch(`http://127.0.0.1:${appPort}/api/chat/bootstrap`)).json();
  assert.equal(bootstrap.config.availability, "unavailable");
  const response = await fetch(`http://127.0.0.1:${appPort}/api/chat/messages`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ content: "停止中には保存されない相談" }),
  });
  assert.equal(response.status, 503);
  assert.deepEqual(await response.json(), { error: "AI相談は現在停止中です。ローカルLLMを起動してから再度開いてください。" });
  const after = await (await fetch(`http://127.0.0.1:${appPort}/api/chat/bootstrap`)).json();
...
test("SlackとMisskeyは共通v3 promptでactionとaspirationをpending候補へ分ける", async (context) => {
  ...
  const fakeLlm = createServer(async (request, response) => {
    ...
    const sourceBody = input.split("SOURCE_BODY:\n")[1];
    const aspiration = input.includes("SOURCE_KIND: misskey");
    response.writeHead(200, { "content-type": "application/json" });
    response.end(JSON.stringify({
      choices: [{
        message: {
          role: "assistant",
          content: JSON.stringify({
            document_summary: aspiration ? "自宅の本を検索したいという希望。" : "Slack投稿の確認作業。",
            candidate_proposals: [{
              proposal_type: aspiration ? "aspiration" : "action",
              title: aspiration ? "自宅の本を横断検索したい" : "Slack投稿を候補として確認する",
              summary: aspiration ? "自宅の本を横断検索できるようにしたい。" : "Slack投稿を候補として確認する。",
              todo: aspiration ? sourceBody : "Slackの投稿を候補として確認する",
              kind: aspiration ? "idea" : "todo",
              schedule: "候補なし",
              confidence: aspiration ? "medium" : "high",
              missing: [],
              tags: [],
              evidence_quotes: [sourceBody],
            }],
  ...
  const slack = await localRequest("/api/import/slack", {
    method: "POST",
    body: JSON.stringify({
      channelName: "memo-ideas",
      channelUrl: "slack://memo-ideas",
      messages: [{ ts: "1", text: "Slackの投稿を候補として確認する。" }],
    }),
  });
  assert.equal(slack.imported, 1);
  assert.equal(slack.syncRun.source, "slack");

  await localRequest("/api/admin/sources/misskey", {
    method: "PATCH",
    body: JSON.stringify({ enabled: true }),
  });
  const wish = "なんとなく、いつか自宅の本を横断検索できるようにしたい。";
  const misskey = await localRequest("/api/import/misskey", {
    method: "POST",
    body: JSON.stringify({ notes: [{ id: "note-1", text: wish, url: "misskey://note/1" }] }),
  });
  assert.equal(misskey.imported, 1);
  assert.equal(misskey.syncRun.source, "misskey");

  const hub = await localRequest("/api/bootstrap");
  const slackCandidate = hub.candidates.find((candidate) => candidate.source === "slack");
  const misskeyCandidate = hub.candidates.find((candidate) => candidate.source === "misskey");
  assert.equal(slackCandidate.kind, "todo");
  assert.equal(slackCandidate.status, "pending");
  assert.equal(misskeyCandidate.kind, "idea");
  assert.equal(misskeyCandidate.todo, wish);
  assert.equal(misskeyCandidate.status, "pending");
  assert.equal(hub.executionLinks.length, 0);
  assert.equal(received.length, 2);
  assert.ok(received.every((payload) => payload.messages[0].content.includes("threadline-candidate") || payload.messages[0].content.includes("source-neutral intake analyst")));
});
...
test("ローカルLLMエージェントのcontext toolは指定scopeだけを読み取り、回答生成を継続する", async (context) => {
  ...
  const fakeLlm = createServer(async (request, response) => {
    ...
    if (request.method === "GET" && request.url === "/v1/models") {
      response.writeHead(200, { "content-type": "application/json" });
      response.end(JSON.stringify({ data: [{ id: "tool-test-model" }] }));
      return;
    }
    if (request.method !== "POST" || request.url !== "/v1/chat/completions") {
      response.writeHead(404, { "content-type": "application/json" });
    ...
    received.push(payload);
    response.writeHead(200, { "content-type": "application/json" });
    if (payload.messages?.[0]?.content?.includes("source-neutral intake analyst")) {
      response.end(JSON.stringify({
        choices: [{ message: { role: "assistant", content: JSON.stringify({ document_summary: "状況確認。", candidate_proposals: [] }) } }],
      }));
      return;
    }
  ...
  assert.match(received[2].messages.at(-1).content, /SOURCE_BODY:\n今のタスク状況を見て$/);
});
...
test("GOは実Vikunja契約でtaskを1件だけ作り、署名付きWebhookを反映する", async (context) => {
  ...
  const fakeVikunja = createServer(async (request, response) => {
    ...
    if (request.method === "DELETE" && request.url === "/api/v1/tasks/77") {
      fakeTask = null;
  ...
  await writeFile(
    join(vault, "tasks", "active.md"),
    "---\nstatus: active\n---\n\n# 運用移行\n\n## Next Actions\n\n1. records/ に移行要約を作る。\n",
    "utf8",
  );
  const rebuilt = await localRequest("/api/admin/rebuild-knowledge-vault-candidates", {
    method: "POST",
    body: JSON.stringify({ root: vault, targets: ["tasks"] }),
  });
  assert.equal(rebuilt.deletedTasks, 1);
  assert.equal(rebuilt.reset.candidates, 1);
  assert.equal(rebuilt.reset.decisions > 0, true);
  assert.equal(rebuilt.import.imported, 1);
  const afterRebuild = await localRequest("/api/bootstrap");
  assert.equal(afterRebuild.candidates.length, 1);
  assert.equal(afterRebuild.candidates[0].title, "records/ に移行要約を作る");
  assert.equal(afterRebuild.log.length, 0);
  assert.equal(afterRebuild.executionLinks.length, 0);
  assert.equal(afterRebuild.executionTaskStates.length, 0);
  assert.equal(afterRebuild.syncEvents.length, 0);
});
...
test("P0受入チェックはB01-B05の後にR06/R07を定常作業として置き、配信済みなら視覚受入を保持する", async () => {
  ...
  assert.ok(checklist.indexOf("{id:'B05'") < checklist.indexOf("{id:'R06'"));
  ...
  assert.match(checklist, /2026-07-15の洗い替え・再配信後/);
  assert.doesNotMatch(checklist, /for \(const id of \['R06','R07'\]\).*result:'failed'/);
  assert.match(checklist, /const staleR06R07Comments = new Set/);
  assert.match(checklist, /staleR06R07Comments\.has\(state\[id\]\?\.comment\)/);
});
...
test("完成度星取表は進行中の工程・現在のブロッカー・次の作業を表示する", async () => {
  ...
  assert.match(scorecard, /R06\/R07/);
  assert.match(scorecard, /R06\/R07は配信資材を更新した回に先頭で再実施する定常ゲート/);
  assert.match(scorecard, /P0本体も未受入のUI\/UX・連携・安全を満点にしない/);
  assert.match(scorecard, /Linuxは候補・判断・execution link・取込系テーブルが0件のclean baseline/);
  assert.match(scorecard, /WindowsのOllamaは利用者依頼により停止中/);
  assert.match(scorecard, /Hub左レールはダッシュボード／簡易日程／タスクキュー／ワークビュー／簡易管理へ再編/);
  assert.match(scorecard, /B04\/U03: 実データ変更の直前確認後だけ、GO・編集・不要・アーカイブを操作IDで照合/);
  assert.match(scorecard, /次の作業/);
  assert.match(scorecard, /scorecard-current-work/);
});

test("完成度星取表は再配信済みR06/R07を配信ブロッカーとして固定しない", async () => {
  const scorecard = await readFile(join(webRoot, "..", "..", "docs", "imp", "p0-p1-completion-assessment-2026-07-12.html"), "utf8");

  assert.match(scorecard, /更新: 2026-07-17/);
  assert.match(scorecard, /2026-07-17の安全再配信後/);
  assert.match(scorecard, /再配信操作は不要/);
  assert.doesNotMatch(scorecard, /候補19 \/ decision 9 \/ execution link 4/);
  assert.doesNotMatch(scorecard, /赤: 定常R06\/R07/);
});

test("完成度星取表はP0からP1への能力増分とP1後も残る未対応を示す", async () => {
  const scorecard = await readFile(join(webRoot, "..", "..", "docs", "imp", "p0-p1-completion-assessment-2026-07-12.html"), "utf8");

  assert.match(scorecard, /段階別にできること／まだできないこと/);
  assert.match(scorecard, /P0追加完了/);
  assert.match(scorecard, /P1完了[\s\S]*現行採用範囲/);
  assert.match(scorecard, /Slack／Misskeyの本人投稿を定期取得/);
  assert.match(scorecard, /複数Hub ProjectとVikunja Projectの所属・GO先を冪等に連携/);
  assert.match(scorecard, /AIが候補を自動GOし、実データを一括変更する/);
  assert.match(scorecard, /PostgreSQLを正本にした高可用な共有データ基盤へ移行する/);
  assert.match(scorecard, /当該フェーズの受入まで終わったときに利用できる/);
});

test("P0進行正本は最新HubとTasksのLinux配信を完了扱いにする", async () => {
  const blockerAudit = await readFile(join(webRoot, "..", "..", "docs", "imp", "p0-blocker-audit-2026-07-15.md"), "utf8");
  const implementationTasks = await readFile(join(webRoot, "..", "..", "docs", "imp", "imp-tasks.md"), "utf8");

  assert.doesNotMatch(blockerAudit, /残るのは、最新版配信/);
  assert.match(blockerAudit, /B01は完了済みとして先頭に表示する/);
  assert.match(blockerAudit, /p0-frontend-acceptance-checklist-2026-07-12\.html/);
  assert.doesNotMatch(blockerAudit, /## ブロッカー表/);
  assert.doesNotMatch(blockerAudit, /最新Hub配信後、確認を得て1batch取込/);
  assert.match(implementationTasks, /## 2026-07-12 Linux配信（配信完了・実機受入中）/);
  assert.match(implementationTasks, /- \[x\] Linux上でHub `pj-general` を再buildし/);
  assert.match(implementationTasks, /- \[x\] Linux上でVikunja custom imageを別tagでbuildし/);
  assert.doesNotMatch(implementationTasks, /現在のCodex環境からは `unibell4@192\.168\.0\.200` のSSH公開鍵認証が通らない/);
});

test("未実装の複数Project連携はP1開始候補としてP0受入から分離する", async () => {
  const scorecard = await readFile(join(webRoot, "..", "..", "docs", "imp", "p0-p1-completion-assessment-2026-07-12.html"), "utf8");
  const plan = await readFile(join(webRoot, "..", "..", "docs", "imp", "p0-multi-project-linkage-design-2026-07-12.md"), "utf8");

  assert.match(scorecard, /phase:'P1', name:'複数Hub Project ↔ Vikunja Project連携基盤'/);
  assert.doesNotMatch(scorecard, /phase:'P0追加', name:'複数Hub Project ↔ Vikunja Project連携基盤'/);
  assert.match(plan, /P1候補（旧P0追加）/);
  assert.match(plan, /## P1受入/);
});
...
test("P0ブロッカーB01からB05は受入チェックの先頭へ実行順で統合される", async () => {
  const checklist = await readFile(join(webRoot, "..", "..", "docs", "imp", "p0-frontend-acceptance-checklist-2026-07-12.html"), "utf8");
  const blockerAudit = await readFile(join(webRoot, "..", "..", "docs", "imp", "p0-blocker-audit-2026-07-15.md"), "utf8");

  assert.match(checklist, /P0完了ブロッカー（B01から順に確認）/);
  assert.match(checklist, /id:'B01'[\s\S]*id:'B02'[\s\S]*id:'B03'[\s\S]*id:'B04'[\s\S]*id:'B05'[\s\S]*id:'R06'/);
  assert.match(checklist, /B02[\s\S]*P0必須として到達性を回復[\s\S]*縮退受入/);
  assert.match(checklist, /B03[\s\S]*R06[\s\S]*R07[\s\S]*RV01[\s\S]*RV05[\s\S]*U05/);
  assert.match(checklist, /B04[\s\S]*U03[\s\S]*operation ID/);
  assert.match(checklist, /B05[\s\S]*実取込GO[\s\S]*pending候補[\s\S]*GOは行わない/);
  assert.match(checklist, /B01:\{result:'done'/);
  for (const id of ["B02", "B03", "B04", "B05"]) assert.match(checklist, new RegExp(`${id}:\\{result:'failed'`));
  assert.match(checklist, /section\.id=task\.id/);
  assert.doesNotMatch(blockerAudit, /## ブロッカー表/);
  assert.match(blockerAudit, /p0-frontend-acceptance-checklist-2026-07-12\.html/);
});
