...
test("完成度星取表は進行中の工程・現在のブロッカー・次の作業を表示する", async () => {
  ...
  assert.match(scorecard, /R06\/R07は、HubまたはTasksの配信資材を更新した回に必ず先頭で実施する定常ゲート/);
...
test("完成度星取表は再配信済みR06/R07を配信ブロッカーとして固定しない", async () => {
  ...
  assert.match(scorecard, /更新: 2026-07-14/);
  assert.match(scorecard, /2026-07-14の現行bundle再配信/);
