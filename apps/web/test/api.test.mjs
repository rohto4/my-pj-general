...
test("P0受入チェックはR06/R07を定常作業として最上段に固定し、配信済みなら視覚受入を保持する", async () => {
  ...
  assert.match(checklist, /2026-07-14に現行bundleをLinuxへ再配信/);
...
test("P0進行正本は最新HubとTasksのLinux配信を完了扱いにする", async () => {
  ...
  assert.match(blockerAudit, /\| B01 \| 最新Hub sourceのLinux反映 \| 完了 \|/);
