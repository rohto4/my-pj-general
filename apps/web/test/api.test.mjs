...
test("完成度星取表は進行中の工程・現在のブロッカー・次の作業を表示する", async () => {
  ...
  assert.match(scorecard, /Linuxは候補・判断・execution link・Vikunja taskが0件のclean baseline/);
...
test("完成度星取表は再配信済みR06/R07を配信ブロッカーとして固定しない", async () => {
  ...
  assert.match(scorecard, /更新: 2026-07-15/);
  assert.match(scorecard, /2026-07-15の安全再配信後/);
