...
- Web / Slack / chatはadapterから直接共通`candidates`へ正規化する。knowledge-vault AI batchだけは、許可断片・AI run・提案をlineage tableへ保存してからaccepted提案をcandidate化する。
...
- Vault AI提案は決定的validatorを通過したものだけ`candidates.status=pending`へ写像し、LLMはGOしない。
