...
- Web manualは直接candidateへ入る。Slack / Misskey / knowledge-vault / chatは共通v3でaction / aspirationを検証する。Slack / Misskeyはacceptedをpendingへ直接写像し、chatは提案カードの受理後にpendingへ写像する。knowledge-vault AI batchだけは、許可断片・AI run・提案をlineage tableへ保存してからaccepted提案をcandidate化する。
