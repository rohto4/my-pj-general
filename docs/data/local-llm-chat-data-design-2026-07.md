...
| `POST /api/chat/messages` | availabilityが`ok`のときだけユーザー発言を保存して相談回答を生成し、別呼出で直近user messageだけを共通v3へ渡して候補提案を返す。停止中は保存せず`503`を返す。agentが要求した読み取り専用context toolは相談回答側だけでbackendが仲介する |
