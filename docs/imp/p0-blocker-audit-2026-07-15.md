...
| 4入口共通の精度優先候補提案prompt | `apps/web/prompts/threadline-candidate-proposal-v2.txt`、`apps/web/candidate_proposal.py`、`apps/web/vault_intake.py` | action / aspiration、完全一致引用、aspiration非具体化、完了除外、タグ・日付、秘密らしい行、LLM失敗時action限定fallbackを自動回帰。旧v1の実Vault dry-runは3 accepted / 2 heldであり、v2の実データ品質はB05で再受入する |
