...
- [x] 共通v2 promptのPython/Node読込点、LLM system message、validator、pending保存までを実行位置図へ追加した。Vaultの2本のsequenceは明示light theme・boxなし・下側actorなしへ修正し、閲覧themeに依存する白文字化と過度な縦長化を防ぐ回帰を追加した。
- [x] `redeploy-p0-frontend.ps1`の既定鍵を登録済み`codex_pjserver_ed25519`へ統一し、通常のsudo不要source-only再配信とAPI/SQLite読取り検証はCodexが直接行う。sudo、secret、実データ、DB/files/volume操作はユーザー境界として維持する。
...
- [x] 原文断片・content hash・モデル/プロンプト版・検証理由を追跡できるbatch schemaとSQLite lineage/proposal schemaを定義した。LLMは候補提案までとし、GO、Vikunja登録を直接行わせない。
...
- [x] Linuxへ洗い替え更新を再配信し、Vikunja task・Hub候補・判断/連携/source同期監査を空にした。旧Linuxローカルscanは`scanned=0 / created=0`で、空の同期runだけを開始点として残した。
- [ ] 新しいWindows AI batch経路で実Vaultを取り込み、由来付きpending候補を新しいレビュー対象として登録する。実行前にユーザー確認を得る。
...
- [ ] 定常R06: 2026-07-14の現行bundle再配信とAPI/読取り確認は完了した。Tasks本文の青・淡青統一とタイトル面の最終見た目・操作感は受入HTMLの黄項目でユーザーが確認する。将来HubまたはTasksの配信資材を更新した回は、Codexが登録済み専用鍵で`redeploy-p0-frontend.ps1`を直接実行し、API/SQLite読取り確認まで閉じる。Linux環境が最新版でなければ受入HTMLを赤へ戻して再配信を先に行う。DB/files/volume削除・再インポートは禁止。
...
- [x] Linux上でHub `pj-general` を再buildし、`/api/health`、`/api/bootstrap=200`、SQLite integrity `ok`、本流テーマsourceを確認した。
- [x] Linux上でVikunja custom imageを別tagでbuildし、`/api/v1/info=200`を確認した。stable `v2.3.0`へimage参照だけを戻せるrollback契約とscript回帰を維持する。
- [ ] Linux実機でHome / Dashboard / Inbox List / Table / Kanban / Gantt / Task detailとHub導線を受入確認する

Linux側の実行手順は `docs/guide/linux-listening-lounge-deploy.md` を正本とする。2026-07-15に専用SSH鍵と安全再配信scriptでHub / Tasksの再build・切替・API確認まで完了した。残るのは利用者による実画面の見た目・操作感受入である。
...
- [x] P0改善: AI要約・抜粋prompt、既存タグmaster限定、TODO案を優先表示する候補タイトル、根拠引用、lineage、LLM停止時縮退を`knowledge-vault AI取込`のarch/spec/data/opsと版管理promptへ分離し、Windows collector・Linux importer・回帰へ実装した。実データのpending候補登録だけを確認待ちとする。
