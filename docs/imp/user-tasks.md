...
1. HTML先頭のB01で、2026-07-17にHub / Tasksを現行資材へsource-only再配信し、API 200、SQLite integrity `ok`、候補・判断・link・取込系テーブル0件のclean baselineが完了済みであることを確認する。追加操作はない。
...
Tasks本文にHub由来のオレンジが残る実機差分を確認した。R06でTasksのprimary/link/focus/選択行/left rail/button/card/task/dashboard/calendarをTasks青・淡青へ統一するsource修正を行い、R07でプロジェクト・ラベル・チームを`マスタ管理`へ統合した。2026-07-17に現行Hub / Tasks資材をLinuxへ再配信し、Hub `/api/bootstrap=200`、Tasks `/api/v1/info=200`、SQLite integrity `ok`、候補0 / decision 0 / execution link 0を確認した。P0を閉じるにはB02、R06/R07、RV01〜RV05、U03/U05の最終受入と、v2→v3比較レビュー後のVault AI batch実データ品質確認が必要である。P1本実装はこれらの受入後に再開する。
...
2026-07-16に共通promptをv3へ圧縮し、実Vault 1文書の`-RequireLlm -DryRun`はaccepted 2 / held 0 / fallback 0となった。実体と差異は`docs/imp/prompt-v2-v3-comparison-2026-07-16.html`で確認する。ここでv3をレビューした後だけ、次の実取込へ進む。
