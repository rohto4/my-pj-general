...
最新HubをLinuxへ再配信した後、実Vault batchを1回取り込む操作はpending候補とlineageを実データへ追加する。Codexは実行直前に確認を取り、実行後は候補の原文忠実度、固有名詞・path、TODO具体性、不要候補率を一覧で示す。ユーザーが見るのはHubの候補本文だけで、GOは別確認とする。
