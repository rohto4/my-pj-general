...
1. 2026-07-14にHub/Tasksの現行bundleはLinuxへ再配信済みであり、R06/R07は赤ではなく最終見た目・操作感を判定する黄項目である。HubとTasksを強制再読込して確認する。以後にHubまたはTasksを再配信した回は、Linux環境が最新版でなければHTMLでR06/R07を「未達」にして赤へ戻してから再配信する。
2. R06では本文の主要ボタン、link、focus、選択行、左端線、card/task/dashboard rail、カレンダー上端にオレンジが残らず、Tasks青・淡青になっていることを確認する。R07では左レールの`マスタ管理`を開き、プロジェクト・ラベル・チームが一画面で読め、各`追加`と既存の詳細／編集導線が動くことを確認する。
3. R06/R07の確認後、R05と同じHTMLのRV01から順に実画面受入を再開する。RV01〜RV04は受領済みの指摘をsource実装へ反映した結果の再確認、RV05は1438×715に揃えたbefore/after証跡の再取得・比較である。U03は実データを変更するため、実行前にCodexへ確認する。

SSH・sudoが必要な作業は、ユーザーの対話SSH端末で行う。パスワード、env全文、secret、token、Cookie、秘密鍵は共有しない。

## 現在のユーザー確認

Tasks本文にHub由来のオレンジが残る実機差分を確認した。R06でTasksのprimary/link/focus/選択行/left rail/button/card/task/dashboard/calendarをTasks青・淡青へ統一するsource修正を行い、R07でプロジェクト・ラベル・チームを`マスタ管理`へ統合した。2026-07-14のLinux再配信後、Hub `/api/bootstrap=200`、Tasks `/api/v1/info=200`、候補19 / decision 9 / execution link 4、Hub/Tasks/管理画面のbrowser error 0を確認済みである。P0を閉じるにはR05、R06/R07、RV01〜RV05、U02/U03/U05の最終受入が必要である。P1本実装はこれらの受入後に再開する。

次にユーザーが行うのは、受入HTML先頭の定常R06/R07で再配信後の最終見た目・操作感を確認すること（再配信操作は不要）。その後にR05、RV01〜RV05/U05のVikunja画面最終判定へ進む。U02の新規AI相談候補とU03の実データ判断操作は、実行前の確認後に行う。
