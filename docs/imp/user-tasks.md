...
1. Hub左レール再編を含む新しいHub bundleを作成したため、受入HTMLの先頭に固定した定常作業R06/R07は現在赤である。同じ一行scriptでHub/Tasksを再配信し、Tasksを強制再読込する。以後もHubまたはTasksを再配信した回は、Linux環境が最新版でなければHTMLでR06/R07を「未達」にして赤へ戻してから再配信する。
...
Tasks本文にHub由来のオレンジが残る実機差分を確認した。R06でTasksのprimary/link/focus/選択行/left rail/button/card/task/dashboard/calendarをTasks青・淡青へ統一するsource修正とbundle作成を完了した。R07でプロジェクト・ラベル・チームを`マスタ管理`へ統合した。P0を閉じるにはR06/R07再配信・実機確認後のR05、RV01〜RV05、U02/U03/U05の受入が必要である。P1本実装はこれらの受入後に再開する。

次にユーザーが行うのは、受入HTML先頭の定常R06/R07でLinux反映状態を確認すること。その後にR05、RV01〜RV05/U05のVikunja画面最終判定へ進む。U03の実データ判断操作は、実行前の確認後に行う。
