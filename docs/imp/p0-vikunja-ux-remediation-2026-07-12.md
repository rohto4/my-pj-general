...
- Tasks左レールのアイコンはHubと同じ記号/輪郭/24px枠を使い、Hubは銅オレンジ、Tasksは青へ変更するfork source修正を追加済み。2026-07-14に現行bundleをLinuxへ再配信し、Tasks `/api/v1/info=200`、左レールと`マスタ管理`三セクションの読み取り確認まで完了した。最終の見た目・操作感と幅別受入はユーザー確認として未完了である。
