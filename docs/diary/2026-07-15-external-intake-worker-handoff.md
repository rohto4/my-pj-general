...
- `docs/arch/windows-vault-ai-intake-architecture-2026-07.md`の責務表を、Windows収集からLinux pendingまでと、利用者確認からGO後Vikunja登録までの2本のMermaidシーケンス図へ置換した。閲覧側themeに依存する白文字化と縦長化は、明示light theme・boxなし・下側actorなしへ修正済みである。
- `docs/spec/ai-candidate-proposal-contract-p0.md`へ、共通v2 promptファイルがWindows Python / Hub NodeからLLM system messageへ組み込まれ、validatorとpending境界へ流れる実行位置図を追加した。
- 通常のHub / Tasks source-only再配信は、登録済み専用鍵とsudo不要scriptを使ってCodexが直接実行できる。sudo、secret/env、実データ変更はこの委任に含めない。
...
- sudoを伴うLinux操作、systemd登録・有効化、secret/env設定・全文表示、実データ変更（通常のsource-only安全再配信はCodexが直接実行可）
...
通常のHub / Tasks source-only安全再配信は、登録済み専用鍵とsudo不要scriptでCodexが直接実行してください。実データ変更、外部書込み、sudo、secret/env設定、systemd有効化は直前に確認を取り、P0受入とP1実装を同じcommitへ混ぜないでください。状態・タスク・設計・完了記録をdocs管理ルールに従って同期し、検証後にrohto4/my-pj-generalのmainへcommit/pushしてください。
```
