...
> 状態: 将来の自動分類・業務object分離候補。現行P0はsource adapterの候補化とAI相談のユーザー確認付き提案を実装し、GOはkindにかかわらずVikunja taskを作る。現行正本は `docs/product/p0-overall-workflow-2026-07.md` と `docs/spec/confirmation-queue-p0.md`。

作成日: 2026-07-02

## 目的

この文書は、将来 `正規化イベント` からAIが整理候補を作り、複数の業務objectへ昇格する場合の候補設計を定義する。現行P0で完全実装した仕様ではない。

ここでは AI の prompt 詳細や DB schema は確定しない。P0 で必要な確認点、状態、責務境界を固定する。

## 前提

- 現行入口は Web、Slack payload、knowledge-vault、AI相談。MisskeyはP1 PoC。
...
- 部分自動確定はP1で実運用判断データを得た後、dry-runから評価する。
...
- 編集、GO、不要、アーカイブの送信ごとに、クライアントは安全な形式の操作IDを1つ生成する。
- API応答の`decision.operationId`、最新`bootstrap.log[].operationId`、SQLite `decisions.note`の`operation:<ID>`は同一IDを返す。
- 画面の判断ログには操作IDを表示し、利用者は実データ受入時にHTTP応答・画面・SQLiteの三者を同じIDで照合できる。
- 操作IDは認証情報・本文・タグ・外部tokenを含めない。確認ダイアログで取消した操作は、候補・件数・判断ログを更新せず、操作IDも永続化しない。
...
| L3 | 条件付きで自動登録する | P1以降、dry-runとユーザー判断後だけ |
...
- 新規入口候補の抜粋は、Markdown見出しや罫線ではなく本文行を優先する。要約は入口名・相対参照・本文の事実を短く示し、固定の説明文だけにしない。
- AI相談の候補要約は、入力にない推測や定型の導入句を含めない。TODO案は画面上の実行タイトルとしてそのまま読める具体的な行動句を優先し、曖昧な「確認する」「整理する」だけの文言を避ける。
