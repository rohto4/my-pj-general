...
- Tasks UIのforkは表示・導線・既存viewの操作を担当し、Vikunja backend API、認証、権限、Webhook、Hub候補の正本を変更しない。
- 配信はsource-only bundleのhash照合、別tag保持、build、両APIのhealth確認を通す。具体手順は `docs/guide/linux-listening-lounge-deploy.md` を正本とし、Tasks画面から再配信・rollbackを実行する導線は作らない。
- 配信不具合または回帰時は、custom imageのtagだけをstable `v2.3.0`へ戻す。DB schema変更を同時に入れず、Hub SQLite、Vikunja DB、files、candidateを削除・再インポートしない。backup / rollbackの運用正本は `docs/ops/p0-operations-runbook-2026-07.md` である。
- 自動回帰は対象view/component/SCSSと対応するfork test、型検査、style検査を使う。実画面はLogin、Home、Dashboard、Inbox、List、Table、Kanban、Gantt、task detailを幅別に受入し、同一1438x715・同一scroll位置のbefore/afterを保存する。
- 色、質感、情報密度、ロゴの最終的な見た目・操作感はユーザーの実画面確認が必要である。この契約は画面の機能・状態・責任分界を固定するもので、視覚スタイルの承認を自動回帰で代替しない。
