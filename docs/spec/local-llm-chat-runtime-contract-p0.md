...
| `POST /api/chat/messages` | ユーザー発言、LLM回答、共通v3で検証済みの提案を返す | 空入力は`400`。providerが`ok`以外なら、保存前に一般化した`503`を返す。回答開始後のprovider失敗は`502`と保存済みユーザー発言を返す。候補抽出だけの失敗は回答を返し、提案0件とする | availability確認を通過したときだけ、ユーザー発言を回答LLM呼出前に保存する。回答成功後、直近user messageだけで候補抽出し、accepted提案だけを保存する |
...
| 直近user message限定の共通v3抽出、action / aspiration | `apps/web/test/test_candidate_proposal.py`と`apps/web/test/api.test.mjs` |
