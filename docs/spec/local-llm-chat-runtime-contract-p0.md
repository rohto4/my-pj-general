...
LLMは回答と候補提案を行う補助である。相談回答と候補抽出は別呼出とし、候補抽出は`docs/spec/ai-candidate-proposal-contract-p0.md`に従う。候補を直接作成・編集・GOしたり、Vikunja taskを直接作成したりしない。

## HTTP契約

| HTTP境界 | 成功 | 失敗・非動作 | 永続化境界 |
| --- | --- | --- | --- |
| `GET /api/chat/bootstrap` | thread、直近message、提案、公開可能な接続状態、許可済みcontext要約を返す | `config.availability`は`ok` / `unavailable` / `disabled`。依存が未設定または到達不能でも、Hubの候補・Tasksを変えない | 必要なら既定threadを作る。API key、secret、raw headerは返さない |
| `POST /api/chat/messages` | ユーザー発言、LLM回答、共通v2で検証済みの提案を返す | 空入力は`400`。providerが`ok`以外なら、保存前に一般化した`503`を返す。回答開始後のprovider失敗は`502`と保存済みユーザー発言を返す。候補抽出だけの失敗は回答を返し、提案0件とする | availability確認を通過したときだけ、ユーザー発言を回答LLM呼出前に保存する。回答成功後、直近user messageだけで候補抽出し、accepted提案だけを保存する |
...
回答本文には提案ブロックを要求しない。候補抽出の入力は直近user messageだけであり、assistant回答、過去会話、read-only tool contextを根拠にしない。候補のtitle、summary、todo、kind、schedule、confidence、missingは通常のcandidate形式へ写像する。
...
- OllamaまたはOpenAI互換endpointが停止・未設定でも、Hub、確認待ち、Vikunja導線、既存会話履歴は維持する。AI相談の入力・送信・サイド窓口は閉じ、再起動案内を表示する。APIも保存前に一般化した`503`を返す。
...
| 直近user message限定の共通v2抽出、action / aspiration | `apps/web/test/test_candidate_proposal.py`と`apps/web/test/api.test.mjs` |
| scope付きread-only toolと回答継続 | 同testの「ローカルLLMエージェントのcontext toolは指定scopeだけを読み取り、回答生成を継続する」 |
| 到達不能依存の短時間`degraded`、AI相談クローズ、保存前503、秘密非露出 | 同testの「healthは設定済み依存へ到達できない時も短時間でdegradedを返す」「ローカルLLM停止中のAI相談は送信前に閉じ、会話を保存しない」 |
