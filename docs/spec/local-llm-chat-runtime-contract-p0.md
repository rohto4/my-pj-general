...
| `POST /api/chat/messages` | ユーザー発言、LLM回答、共通v2で検証済みの提案を返す | 空入力は`400`。回答provider失敗は`502`と保存済みユーザー発言を返す。候補抽出だけの失敗は回答を返し、提案0件とする | ユーザー発言は回答LLM呼出前に保存する。回答成功後、直近user messageだけで候補抽出し、accepted提案だけを保存する |
