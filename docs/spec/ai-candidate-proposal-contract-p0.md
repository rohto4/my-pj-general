...
- Slack / Misskey定期workerは`candidate_proposal.build_request()`を再利用し、別promptを作らない。実装済みの`workers/sync/llm_client.py`と`proposal_pipeline.py`も同じv2 prompt / validatorだけを通す。
...
| Slack | connectorまたは手動payloadが対象投稿を渡す | `POST /api/import/slack`がv2を実行し、acceptedだけ`SLAI-*` pending候補 | payload以降とLinux polling workerを実装済み。実token・実API・実SQLite書込み・timerは未実施 |
| Misskey | 将来connectorが対象noteを渡す | 有効化済みsourceへの`POST /api/import/misskey`がv2を実行し、acceptedだけ`MKAI-*` pending候補 | payload以降とLinux差分workerを実装済み。sourceは既定無効、実接続・実データ取込・timerは未実施 |
...
| Slack / Misskeyのpending写像、held、冪等 | `apps/web/test/test_source_sync.py`、`workers/sync/test_external_intake.py` |
